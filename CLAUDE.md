...
Implementation home for the agent-memory system. Stream A is the Rust substrate (canonical Markdown+YAML files, derived SQLite/FTS/vector indexes, per-device JSONL events, git as sync transport). Stream B is the local `memoryd` daemon and MCP bridge that fronts it. Stream C adds deterministic governance for structured writes, supersession, forgetting, and review visibility. Stream D adds the privacy classification + age-encryption boundary in front of Stream A writes. Stream E adds passive startup/delta recall through daemon/MCP/CLI surfaces.

## Current status (as of 2026-05-01)

**Streams A, B, C, D, E, and F are shipped. Stream F (dreaming) v0.2 shipped after the final release gate from `docs/plans/2026-04-30-stream-f-dreaming.md` against `docs/specs/stream-f-dreaming-v0.2.md`; gate evidence is recorded in `docs/reviews/stream-f-final-gate-report.md`.**
