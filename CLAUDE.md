...
**Streams A and B are shipped. Streams C and D are implemented in the current working tree; Stream D is awaiting Codex's response to a Claude review before commit.**
...
Stream D: Codex landed the privacy crate (`crates/memory-privacy/`) plus substrate integration (`api.rs`/`atomic.rs` honor a new `allow_encrypted_namespace` flag and reject plaintext under `encrypted/`), memoryd handler wiring (every governance write/supersede/note runs through `DeterministicPrivacyClassifier` before disk effects, secret/high-risk identity material is refused before any disk write, detected PII and caller personal/confidential content route to `write_encrypted` through `PrivacyStorageAction`), safe descriptor indexing for encrypted records, explicit `memory_reveal`, CLI admin commands (`memoryd privacy …`, `memoryd privacy-filter …`, `memoryd device …` — none exposed via MCP), and e2e tests (`crates/memoryd/tests/privacy_e2e.rs`) covering secret refusal, URL/date plaintext searchability, encrypted-but-findable/revealable contacts, no raw projection leakage, encrypted forget, fail-closed encrypted supersede, fail-closed encrypted review. Codex wrote three self-reviews (`docs/reviews/stream-d-{correctness,performance,security}-review.md`); Claude added a fourth (`docs/reviews/stream-d-claude-review.md`) flagging URL/date/phone tier-elevation blockers. The implemented fix decouples tier from storage routing: URL/date stay plaintext by default, phone/email/address encrypt at rest without `Personal` tier elevation, and reveal is explicit/audited.
...
## What's on disk (Streams A and B shipped; C and D in working tree, as of 2026-04-29)
