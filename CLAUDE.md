...
**Runtime-loop foundation (2026-06-25→06-28, BUILT + gate-green on branch, pre-merge):** a 6-day single-device dogfood found the runtime loop never closed end-to-end (writes never git-commit → dreaming dead on a no-remote install → no ambient capture → doctor blind). The P0 fix set (F1 commit-on-write, F2 single-device lease, F3 recovery/dedup, F4 doctor) is **built, audited + Codex-reviewed + remediated, and passes the full `scripts/check.sh` end-to-end** on branch `foundation/runtime-loop-closure` (latest `e0dcfd7`, pushed). The live spec is now **`docs/specs/memorum-runtime-loop-foundation-v0.2.md`** (v0.1 retained for history; v0.2 reduced the F3 quarantine-resolve CLI to `--edited`). Plan: `docs/plans/2026-06-25-runtime-loop-foundation-implementation.md` (post-execution amendment at top). **Wave E status (2026-07-03):** the daemon was redeployed live from `perf/daemon-memory` (a superset of this branch — adds the embedding-lifecycle memory reduction, spec `stream-e-passive-recall-v0.7`, and a serve-path tracing subscriber). Foundation gate steps verified live: write→commit (F1 drained the 559-file backlog on first boot), doctor (green except the pre-existing 2-memory `sync_blocked` quarantine awaiting Trey). **Remaining:** the dream step of the gate (`memoryd dream now`), quarantine resolution, and the `main` merge (order: foundation vs perf branch is Trey's call). This foundation is Phase 0 of the v3.0 ambient-recall redesign (`docs/specs/stream-e-ambient-recall-v3.0.md`).
