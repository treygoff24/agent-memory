...
**Memora-lessons arc (2026-07-10→15, CLOSED 2026-07-15):** W0/W1 integrated; W4 four-lane merged dark after failing the honest bar; **W4b re-gate resolved INCONCLUSIVE per pre-registered rule 1** — v2 enrichment content shifts write-time privacy classification under the API-lane fence (39 quarantine flips, 33/120 LongMemEval contexts diverge), so the corpus attribution control failed and no treatment arm ran. Verdict, six-run ledger, diagnosis, and the v2-vs-v2 re-gate option: `docs/reviews/memora-arc/w4b-results.md`. **W4c then cleared the holdout gate and `four_lane_enabled` shipped ON** (uniform lane weights, `2b3cb34`; HEAD `941caa3` "W4c HOLDOUT PASS + four-lane shipped ON"). **B3 metadata-amendment SHIPPED** (spec `91c51ab`, code `86e5124`, candidate-query fix `1a97aa4`): typed `metadata_amend` with hardcoded compile actor, CAS-before-idempotency, tier/Me-floor refusals, encrypted closure-revalidation, single `MetadataAmended` event, dream apply path rewired off governed supersede. **W5 live backfill COMPLETE**: all 898 active/pinned rows in `~/memorum` carry abstraction+cues (codex harness; claude CLI proved unusable for bulk JSON). Arc ledger: `thoughts/memora-build/BUILD-STATE.md`; reviews under `docs/reviews/memora-arc/`.

**Open follow-ups:** (1) the grounding→privacy catch-22 for self-referential governed writes with no public source; (2) cosmetic — `--lane local` switch envelope still prints `approximate_tokens`/`estimated_usd`; (3) dream-scheduler plist still deliberately unwired. Next planned arc after memora: ambient-recall v4.0 (`docs/specs/stream-e-ambient-recall-v4.0.md`).
...
When Trey says "the spec" or "the plan" without a version, he means the latest. Live spec = highest-versioned `docs/specs/stream-X-...-vN.M.md` (older versions stay on disk for history; never mutate them). The parent system spec is `docs/specs/system-v0.3.md`.
