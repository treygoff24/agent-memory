...
function scenarioError(scenario: ApiScenario, route: string): ApiMockResponse | null {
  if (scenario === 'error')
    return jsonResponse(
      { error: 'mock_error', route, message: `${route} failed in error scenario` },
      500,
    );
  if (scenario === 'forbidden403')
    return jsonResponse(
      { error: 'forbidden', route, message: 'Dashboard policy forbids this request.' },
      403,
    );
  ...
  if (scenario === 'unavailable503')
    return jsonResponse(
      { error: 'backend_unavailable', route, message: 'memoryd daemon is not reachable.' },
      503,
    );
  return null;
}
...
function recallHitsPayload(scenario: ApiScenario, url: globalThis.URL) {
  const requestedLimit = Number(
    url.searchParams.get('limit') ?? (scenario === 'heavy' ? 9000 : 80),
  );
  const count =
    scenario === 'empty'
      ? 0
      : Math.min(scenario === 'heavy' || requestedLimit > 500 ? requestedLimit : 80, 9000);
  const summaries = [
...
export function payloadForApiRequest(
  ...
  if (method === 'GET' && url.pathname === '/api/status')
    return jsonResponse(statusPayload(scenario));
  if (method === 'GET' && url.pathname === '/api/entity-graph')
    return jsonResponse(entityGraphPayload(scenario));
  ...
  if (method === 'GET' && url.pathname === '/api/roi') {
    return jsonResponse({
      ...
      refusal_breakdown:
        scenario === 'empty' ? {} : { contradiction: 1, grounding: 7, policy: 2, tombstone: 3 },
      dreaming: {
  ...
  if (method === 'GET' && url.pathname === '/api/reality-check')
    return jsonResponse(realityCheckPayload(scenario));
  ...
  if (method === 'GET' && url.pathname === '/api/recall-hits')
    return jsonResponse(recallHitsPayload(scenario, url));
  if (method === 'GET' && /^\/api\/audit\/[^/]+$/.test(url.pathname))
    return jsonResponse(auditPayload(url.pathname));
  ...
  if (method === 'GET' && url.pathname === '/api/notifications/stream')
    return notificationsStream();
  if (method === 'GET' && url.pathname === '/api/policy-editor')
    return jsonResponse(policyPayload());
  ...
  if (method === 'GET' && url.pathname === '/api/sync-dashboard')
    return jsonResponse(syncPayload(scenario));
