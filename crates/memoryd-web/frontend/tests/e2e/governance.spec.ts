...
test('governance review queue filters consent items and exposes batch actions', async ({
  page,
}) => {
  await page.goto('/?view=governance');
  await page.getByRole('tab', { name: /consent/i }).click();
  await expect(page.getByTestId('governance-view-consent_required')).toContainText(
