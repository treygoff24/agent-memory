...
test('settings tabs expose theme editor and notification controls', async ({ page }) => {
    ...
    await expect(page.getByRole('region', { name: 'Theme editor' })).toContainText('Custom theme preview');
    await page.getByRole('tab', { name: 'Notifications' }).click();
    await expect(page.getByRole('region', { name: 'Notifications' })).toContainText('Daemon health alerts');
});
