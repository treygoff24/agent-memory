...
export function Recall({ events, heavy }: RecallProps) {
  ...
  const sourceEvents = useMemo(
    () => events ?? query.data?.hits.map(fromRecallHit) ?? [],
    [events, query.data],
  );
  ...
  const buckets =
    bucketMode === '30d' ? makeDayBuckets(sourceEvents) : makeHourBuckets(sourceEvents);

  return (
    <div data-testid={`recall-ledger-${urlState}`}>
      {!events && query.isLoading ? <QueryLoadingBanner label="Recall ledger" /> : null}
      <QueryErrorBanner error={query.error} label="Recall ledger" />
      ...
      <div className="panes-2">
        ...
        <div className="pane">
          <div className="pane-scroll">
            <Inspector item={inspectorItemFromRecall(selected)} layout="narrow" />
          </div>
        </div>
      </div>
    </div>
