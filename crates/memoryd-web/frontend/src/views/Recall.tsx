...
import { recallEvents, type RecallEvent } from '../data/fixtures';
...
export function toRecallLedgerEvent(event: RecallEvent, index: number): RecallLedgerEvent {
...
function makeHourBuckets(events: RecallLedgerEvent[]): TimelineBucket[] {
    const counts = Array.from({ length: 24 }, (_, key) => ({ key, label: `${String(key).padStart(2, '0')}:00`, count: 0 }));
...
function makeDayBuckets(events: RecallLedgerEvent[]): TimelineBucket[] {
    const counts = Array.from({ length: 30 }, (_, key) => ({ key, label: `day ${key + 1}`, count: 0 }));
...
export function Recall({ events, heavy }: RecallProps) {
    ...
    const sourceEvents = useMemo(
        () => events ?? (heavyMode ? makeHeavyRecallEvents(9000) : recallEvents.map(toRecallLedgerEvent)),
        [events, heavyMode],
    );
    ...
    const visible = useMemo(
        () =>
            sourceEvents.filter((event) => {
                ...
                if (search && !`${event.memory} ${event.namespace} ${event.agent} ${event.device}`.toLowerCase().includes(search.toLowerCase())) return false;
                return true;
            }),
    ...
    return (
        <div data-testid={`recall-ledger-${urlState}`}>
            <div className="view-header">
                ...
                <span className="view-subtitle">· {sourceEvents.length.toLocaleString()} events · {bucketMode === '24h' ? 'last 24 hours' : 'last 30 days'}</span>
