...
import {
  useRealityCheckQuery,
  useRealityCheckRespondMutation,
  type RealityCheckApiItem,
} from '../api';
import {
  CompletionCard,
  FocusStrip,
...
function toRealityCheckMemory(
  item: RealityCheckApiItem,
  variant: RealityCheckVariant,
): RealityCheckMemory {
...
export function RealityCheck({
  ...
  const resolvedSession = useMemo(
    () =>
      session ??
      (query.data
        ? sessionFromApi(query.data.items, query.data.session_id, resolvedVariant)
        : null),
  ...
  useEffect(() => {
    const onKeyDown = (event: KeyboardEvent) => {
      if (complete || isTextInputTarget(event.target) || isTextInputTarget(document.activeElement))
        return;
      if (event.key === 'Escape') {
        setMode('answer');
        return;
  ...
  if (!resolvedSession) {
    return (
      ...
      >
        ...
        <QueryErrorBanner error={query.error} label="Reality Check" />
      </div>
    );
  }
  ...
  return (
    <div
      ...
      style={{ display: 'flex', flexDirection: 'column', height: '100%' }}
    >
      <QueryErrorBanner error={query.error ?? respondMutation.error} label="Reality Check" />
      <FocusStrip session={resolvedSession} complete={complete} onExit={onExit} />
      <div className="rc-stage">
        ...
        <SessionSidebar session={resolvedSession} complete={complete} />
      </div>
    </div>
  );
}
