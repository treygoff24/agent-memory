...
function decisionFor(item: ReviewQueueItem): GovernanceDecision {
  ...
  if (
    ...
    marker.includes('review_required') ||
    marker.includes('human_required')
  )
...
function severityFor(item: ReviewQueueItem): GovernanceViewItem['severity'] {
  ...
  const marker = `${item.status} ${item.reason ?? ''} ${item.policy_applied}`.toLowerCase();
  if (decision === 'redact_proposed' || marker.includes('block') || marker.includes('secret'))
    return 'block';
...
export function Governance() {
  ...
  const visible = useMemo(
    () =>
      filter === 'all'
        ? items
        : items.filter((item) => item.severity === filter || item.decision === filter),
    [filter, items],
  );
    ...
    (acc, filterOption) => {
      acc[filterOption.id] =
        filterOption.id === 'all'
          ? items.length
          : items.filter(
              (item) => item.severity === filterOption.id || item.decision === filterOption.id,
            ).length;
      return acc;
    },
  ...
  function updateFilter(next: GovernanceFilter) {
    ...
    const nextSelected =
      next === 'all'
        ? items[0]
        : items.find((item) => item.severity === next || item.decision === next);
  ...
  return (
    <div data-testid={`governance-view-${filter}`}>
      ...
      <QueryErrorBanner error={query.error ?? reviewAction.error} label="Governance" />
      <div className="view-header">
        ...
        <div className="filter-pills" role="tablist" aria-label="Governance filters">
      ...
      {checked.size > 0 ? (
        <div className="batch-bar">
          <span className="batch-count" data-testid="governance-batch-count">
            <span className="mono">{checked.size}</span> selected
          </span>
          <span className="sep">·</span>
          <button className="btn-link" onClick={toggleAll} type="button">
            {checked.size === visible.length ? 'deselect all' : `select all ${visible.length}`}
          </button>
          <span className="spacer" />
          <button className="btn primary" onClick={() => submitBatch('approve')} type="button">
            Approve selected
          </button>
          <button className="btn" onClick={() => submitBatch('reject')} type="button">
            Reject selected
          </button>
        </div>
      ) : null}
      <div className="panes-2">
        ...
        <div className="pane">
          <div className="pane-scroll">
            <Inspector item={inspectorItemFromGovernance(selected)} layout="narrow" />
          </div>
        </div>
      </div>
    </div>
