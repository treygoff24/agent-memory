...
import {
  filterItems,
  inboxFilters,
  inspectorItemFromInbox,
  toInboxViewItem,
} from './inboxView/adapter';
...
function kindFromReviewItem(item: ReviewQueueItem): InboxKind {
  const marker =
    `${item.status} ${item.namespace} ${item.reason ?? ''} ${item.policy_applied}`.toLowerCase();
...
export function Inbox({ layout, items: providedItems }: InboxProps) {
  ...
  return (
    <>
      ...
      <QueryErrorBanner error={query.error} label="Inbox" />
      {layoutNode}
    </>
  );
}
