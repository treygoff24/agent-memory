...
function normalizeKind(node: EntityNode): EntityKind {
  ...
  if (marker.includes('place') || marker.includes('home') || marker.includes('office'))
    return 'place';
...
function toEntityViewItem(node: EntityNode, index: number): EntityViewItem {
  ...
  const namespace =
    node.namespace ?? (kind === 'project' ? 'project:agent-memory' : `entity/${kind}`);
  return {
    id: node.id,
    name: node.label,
    kind,
...
export function Entities() {
  ...
  const filtered = useMemo(() => {
    ...
    return sortEntities(
      items.filter((item) => {
        ...
        if (
          queryText &&
          !`${item.name} ${item.kind} ${item.namespaces.join(' ')}`
            .toLowerCase()
            .includes(queryText)
        )
          return false;
  ...
  return (
    <div data-testid={`entities-view-${filter}`}>
      ...
      <QueryErrorBanner error={query.error} label="Entities" />
      <div className="view-header">
        ...
        <div className="filter-pills" role="tablist" aria-label="Entity kind filters">
      ...
      <div className="panes-2">
        ...
        <div className="pane">
          <div className="pane-scroll">
            <Inspector item={inspectorItemFromEntity(selected)} layout="narrow" />
          </div>
        </div>
      </div>
    </div>
