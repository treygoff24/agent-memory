...
function dreamStatus(item: ReviewQueueItem): DreamViewItem['status'] {
  ...
  if (['proposed', 'queued', 'accepted', 'completed', 'dismissed', 'running'].includes(normalized))
    return normalized;
...
export function Dreams() {
  ...
  const items = useMemo(
    () => query.data?.items.filter(isDreamItem).map(toDreamViewItem) ?? [],
    [query.data],
  );
  ...
  return (
    <div data-testid={`dreams-view-${filter}`}>
      ...
      <QueryErrorBanner error={query.error} label="Dreams" />
      <div className="view-header">
        <span className="view-title">Dreams</span>
        <span className="view-subtitle">· {items.length} · last run 03:04 today</span>
        <span className="spacer" />
        <div className="filter-pills" role="tablist" aria-label="Dream status filters">
          {statuses.map((status, index) => (
            <button
              ...
              onClick={() => {
                setFilter(status);
                const next =
                  status === 'all' ? items[0] : items.find((item) => item.status === status);
      ...
      <div className="panes-2">
        <div className="pane left">
          <div className="pane-scroll">
            <DreamList items={visible} selectedId={selected?.id ?? ''} onSelect={setSelectedId} />
          </div>
        </div>
        <div className="pane">
          <div className="pane-scroll">
            <Inspector item={inspectorItemFromDream(selected)} layout="narrow" />
          </div>
        </div>
      </div>
    </div>
