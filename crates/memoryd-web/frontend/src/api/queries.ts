...
export function useRealityCheckHistoryQuery(limit?: number) {
  return useQuery({
    ...
    queryFn: () =>
      apiJson<RealityCheckHistoryResponse>(withParams('/api/reality-check/history', { limit })),
  });
}
...
export function useAuditTemporalQuery(id: string, at?: string) {
  return useQuery({
    ...
    queryFn: () =>
      apiJson<TemporalStateResponse>(
        withParams(`/api/audit/${encodeURIComponent(id)}/temporal`, { at }),
      ),
    enabled: id.length > 0,
  });
}
