...
use crate::recall::render::{render_memory_entry, render_startup_frame, RecallEntry, RenderedRecallSection};
...
pub async fn build_startup_response(
    ...
    let ranking_now = collection.facts.iter().map(|candidate| candidate.row.updated_at).max().unwrap_or_else(Utc::now);
        ...
        selected
            ...
            .map(|candidate| {
                render_memory_entry(&RecallEntry {
                    ...
                    source_kind: format!("{:?}", candidate.candidate.row.source_kind),
...
fn project_body(session_binding: &SessionBinding) -> String {
    match &session_binding.project {
        Some(project) => {
            ...
            format!("- project: {display}\n- namespace: project:{}", project.canonical_id)
        }
