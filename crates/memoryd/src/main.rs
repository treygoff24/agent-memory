...
async fn main() -> anyhow::Result<()> {
    ...
    match cli.command {
        ...
        Command::WriteNote(args) => {
            ...
            let response =
                client::request(socket.clone(), "cli-write-note", RequestPayload::WriteNote { text: args.text })
                    .await?;
...
/// was processed."
fn governance_write_response_promoted_id(response: &memoryd::protocol::ResponseEnvelope) -> Option<String> {
    let ResponseResult::Success(ResponsePayload::GovernanceWrite(write)) = &response.result else {
        return None;
    };
    if !matches!(write.status, memoryd::protocol::GovernanceStatus::Promoted) {
        return None;
    }
    write.id.clone()
}
...
fn write_note_response_id(response: &memoryd::protocol::ResponseEnvelope) -> Option<String> {
...
async fn maybe_emit_first_write_banner(socket: &Path, id: &str) {
    ...
    if memoryd::first_write::should_emit_first_write_banner(&status) {
        let mut stderr = std::io::stderr().lock();
        let _ = memoryd::first_write::emit_first_write_banner(&mut stderr, id);
    }
}
