use memory_privacy::{
    install_runtime_enforcement, DeterministicPrivacyClassifier, PrivacyClassifier, PrivacyEnforcement, PrivacyLabel,
    PrivacyNamespace, PrivacyStorageAction,
};

#[test]
fn double_install_is_rejected_and_first_config_wins() {
    let first = PrivacyEnforcement { classifier: true, encryption: false, masking: false };
    ...
    assert!(
        decision.spans.iter().any(|span| span.label == PrivacyLabel::PrivateEmail),
        "first config should leave classifier enabled and detect the email span: {decision:?}"
    );
}
