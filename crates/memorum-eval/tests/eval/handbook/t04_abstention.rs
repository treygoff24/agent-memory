...
async fn novel_topic_search_and_startup_abstain_without_error() {
    ...
    assert_eq!(search_total(search_json), 0, "novel topic should return zero search hits:\n{search_json}");

    let recall_block = observations.last_startup_block.as_deref().expect("startup recall block captured");
    assert_xml_valid(recall_block).expect("empty-recall startup block is valid XML");
    assert!(!recall_block.contains(NOVEL_TOPIC), "startup must not fabricate novel topic memories:\n{recall_block}");
}
