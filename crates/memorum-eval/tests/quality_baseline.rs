...
//! ## No-baseline skip
//!
//! The baseline is **human-committed only** — the runner never writes it, and
//! `scripts/check-baseline-discipline.sh` enforces the same guard used for other
//! canonical bench JSON. So when the baseline file does not exist yet, this gate
//! **skips cleanly** with a clear message rather than failing: Trey commits the
//! initial baseline after reviewing an emitted JSON report
//! (`memorum-eval-quality --output-file ...`). Once the baseline exists, this
//! test enforces no-regression within a tolerance band.
//!
...
fn report_is_well_formed() {
    ...
    // 56 labeled cases. Abstention = empty essential AND useful: q47-q50 plus
    ...
    // The six q51-q56 keyword/entity cases are scored search-seam probes.
    assert_eq!(report.total_cases, 56, "golden corpus should hold 56 labeled query cases");
...
fn startup_seam_carries_graded_relevance_signal() {
    ...
    // namespace-scoped candidates. Across 50 scored cases it must surface
    ...
    // The search seam is keyword bm25. The natural-language and exact-id cases
    // still document its limitations, but q51-q56 give it positive dynamic
    // range so the baseline can catch search regressions.
    let search = &report.seams["search"];
    assert!(search.mrr > 0.0, "search seam MRR should be > 0 from keyword/entity probes");
    assert!(search.ndcg_at_k["5"] > 0.0, "search seam nDCG@5 should be > 0");
    assert!(search.recall_at_k["5"] > 0.0, "search seam recall@5 should be > 0");
    assert!(
        report.seam_notes.get("search").is_some_and(|n| n.contains("keyword/entity")),
        "search seam must carry its keyword/entity interpretation note"
    );
}
        ...
        GateOutcome::SkippedNoBaseline => {
            ...
            eprintln!(
                "quality baseline not present yet — gate skipped. \
                 Run `cargo run -p memorum-eval --bin memorum-eval-quality -- --output-file /tmp/quality.json` \
