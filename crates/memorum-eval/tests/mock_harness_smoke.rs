...
#[tokio::test]
async fn mock_harness_skips_test_13_because_semantics_are_not_exercised() {
...
#[tokio::test]
async fn mock_harness_skips_test_15_because_semantics_are_not_exercised() {
    let scaffold = fresh_scaffold().await;
...
#[tokio::test]
async fn mock_harness_skips_test_19_without_stream_i_deps() {
    let scaffold = fresh_scaffold().await;
    let outcome =
...
#[tokio::test]
async fn mock_harness_runs_test_19_with_stream_i_deps() {
    let scaffold = fresh_scaffold().await;
    let outcome = MockHarness.run_test(19, &scaffold).expect("mock test #19 should run when stream-i-deps is enabled");
