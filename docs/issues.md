...
## RESOLVED (2026-07-18): shipped `_extras` merge diverges from spec §14.4 (pre-existing, found 2026-07-10)

**Resolution:** Trey ratified option (a): §14.4 now resolves each true `_extras` conflict with a deterministic canonical value-hash tie-break and preserves the loser in diagnostics. `field_rules.rs` applies that tie-break and emits the side-independent `_extras:<key>` diagnostic; `three_way.rs` treats those conflicts as convergent when deriving deterministic diagnostic metadata. Pinning test: `extras_conflict_uses_canonical_value_hash_and_preserves_loser`. Historical analysis retained below.

**Historical analysis (pre-ratification):**
