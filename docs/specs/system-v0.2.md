...
### 10.1 Tier 1 — Full hook integration (v1)

Harnesses that support both **session-start** and **pre-turn** hooks, where Memorum can inject content into the agent's context window before the agent processes the turn.
...
**Admin commands are explicitly rejected from MCP** (Stream B + D). An agent cannot call `memoryd privacy decisions` via MCP. This is a hard boundary: agents see the agent-facing nine; humans run the admin surface.
...
5. **First harness link.** Detects installed harnesses (`claude`, `codex`, `cursor`, …); prompts for which to wire MCP + hooks for. `--link <harness>` / `--link-all-detected` / `--no-link` overrides.
