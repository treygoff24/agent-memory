# Stream E Passive Recall Spec v0.7

**Status:** final implementation contract for Stream E passive recall. Supersedes `stream-e-passive-recall-v0.6.md`.
**Date:** 2026-04-30 (v0.5); 2026-06-10 (v0.6 fusion bump); 2026-06-19 (in-version hook-installation amendment); 2026-07-03 (v0.7 lifecycle-marker bump).

**Revision goal (v0.6 → v0.7):** absorb the embedding-model lifecycle change (daemon memory reduction, `docs/plans/2026-07-03-daemon-memory-reduction-design.md`). Approved by Trey 2026-07-03. The daemon no longer holds the embedding model resident for its lifetime: it loads on demand and unloads after an idle window (`MEMORUM_EMBED_IDLE_UNLOAD_SECS`, default 900s, `0` = legacy hold-forever). This bump:

1. **Adds an eighth degradation marker, `embedding_dormant`** (§16.3): the provider slot is healthy but the model is not currently resident (dormant, or a load in flight); the request degrades to FTS-only for that turn and a coalesced background load is triggered. The first recall after an idle gap longer than the unload window is expected to carry this marker; subsequent recalls in an active session are fused.
2. **Narrows `no_embedding_provider`** (§16.3): it now strictly means the provider is failed or intentionally disabled (`MEMORUM_DISABLE_EMBEDDING_WORKER`, a load failure within its retry backoff, or no loader configured). A healthy-but-unloaded model is `embedding_dormant`, never `no_embedding_provider`.
3. **`active_embedding` changes require a daemon restart** — the active triple is frozen in memory at substrate open; every model (re)load re-probes dimension and re-checks the triple against that frozen value with the same typed-error taxonomy. This codifies the pre-existing de-facto behavior as explicit contract.
4. The version string in policy/manifest/recall-block attributes bumps to `stream-e-v0.7` (`STREAM_E_POLICY` in `crates/memoryd/src/recall/types.rs`).

The "seven-marker" enumeration in the v0.5 → v0.6 revision goal below is retained as accurate history of that bump.
...
<memory-recall version="stream-e-v0.7" harness="codex" session="sess_abc123">
  ...
  <recall-explanation policy="stream-e-v0.7" budget-tokens="3600" used-tokens="1420">
  </recall-explanation>
</memory-recall>
```
...
### 16.3 Degradation ladder — eight markers, never an error, never silent
...
**eight** markers, enumerated here exactly (this enumeration is identical in the
plan's §3 D3, this section, and the implementation's "test each rung"):

| Marker | Trigger |
| --- | --- |
| `embedding_dormant` | The provider lifecycle is healthy but the model is not resident (dormant, or a load in flight). The request degrades for this turn and a coalesced background load is triggered (v0.7). |
| `no_embedding_provider` | The provider is failed or intentionally disabled: `MEMORUM_DISABLE_EMBEDDING_WORKER`, a load failure within its retry backoff, or no loader configured. Healthy-but-unloaded is `embedding_dormant`, not this (narrowed in v0.7). |
