# Stream E Passive Recall Spec v0.2

**Status:** revised draft implementation contract for Stream E passive recall. Supersedes `stream-e-passive-recall-v0.1.md`.
**Date:** 2026-04-30.
...
**Revision goal (v0.1 → v0.2):** close dependency and contract gaps surfaced by the v0.1 review against the shipped Stream A/B/C/D code.

1. Name the Stream D safe-fragment helper as a real public API on `memory-privacy`; remove the implicit dependency.
2. Spec the Stream A `MemoryQuery` extension Stream E needs (status, namespace, passive_recall, updated_since) so the §13 perf gate is reachable without a private Stream E database.
3. Hard-cut the legacy MCP `{ "include_recent": true }` shape; remove the escapeable "tests may keep one compatibility case" clause.
4. Tighten `.memory-project.yaml` parsing: explicit duplicate-key rejection, char-boundary truncation, no `:` in `canonical_id`.
5. Bound the `<recall-explanation>` omission list and require `<memory-delta>` to always emit a wrapper element so downstream tooling can parse unconditionally.
6. Replace `omitted.reason: String` with a closed Rust enum.
7. Replace `cwd` "caller-visible filesystem path" symlink-escape language with simple existence + canonicalization.
8. Add a determinism test, candidate-iteration sort discipline, cold-start perf budget, scaffold-tag budget rule, and an in-process recall-invocation counter for observability.
9. Clarify `since_event_id` `not_implemented` exception to §11's "must not return `not_implemented`" rule.
10. Fix the §10 "CLI arguments" wording — Stream E sanitizes echoed-in-error CLI arguments, not incoming flags.
11. Commit to a project-binding caching policy (recompute every call in v0.1; document explicitly).
...
streams. They are part of the Stream E v0.2 contract; do not implement Stream E
without them.
...
pub struct MemoryQuery {
    ...
    // New in Stream E v0.2:
    pub status: Option<MemoryStatus>,           // active, pinned, candidate, etc.
...
hydration). Defaults preserve current behavior. Stream A index migration adds
indexes on `status`, `namespace`, `retrieval_policy.passive_recall`, and
...
pub enum SafeFragmentDecision {
    ...
    OmitEncryptedBodyHidden,        // hits secret/high-risk class; omit
    OmitReviewPending,              // hits a class flagged for human review
}
```
...
shape is removed in Stream E v0.2.** Requests missing `cwd`, `session_id`, or
...
For Stream E v0.1, project binding is **recomputed on every request**. No cache.
Walking from `cwd` upward and shelling to `git remote get-url origin` is
acceptable per-call I/O for v0.1 traffic patterns (one `memory_startup` per
...
v0.2+ optimization and out of scope here.
...
v0.2 `MemoryQuery` extension (see §1.1) so candidate enumeration is served from
...
A. If Stream A lacks a query needed for efficient collection beyond the v0.2
...
- Omissions are sorted by `(section, reason, id)` for stable ordering.
- The serialized omission list is truncated to **64 entries** in any single
  ...
  counts (`RecallSectionExplanation.budget_used_tokens` plus a future
  `omitted_count` field) remain accurate regardless of truncation.
...
Stream E uses the v0.2 `MemoryQuery` extension (§1.1) to keep candidate
...
disk in v0.1 (see §15 deferral); they reset on daemon restart.
...
- `crates/memory-substrate/tests/memory_query_extension.rs`
  - the v0.2 `MemoryQuery` extension (§1.1) returns expected rows for each new
...
- `crates/memory-privacy/tests/safe_plaintext_fragment.rs`
  ...
  - returns `OmitReviewPending` for review-flagged classes;
