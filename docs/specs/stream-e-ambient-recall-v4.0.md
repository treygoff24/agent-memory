...
Trigger rows live in new index tables behind an `INDEX_SUPPORTED_SCHEMA_VERSION` bump (**6→7** — schema 6 is owned by the Memora-lessons arc's abstraction/cue tables, `docs/specs/2026-07-10-w2-spec-ratification-package.md` §A2, per DP2 of that plan's approval; this spec's P2 migration re-numbered accordingly 2026-07-11), maintained in the same transaction as the existing index upsert (the `upsert.rs` tail alongside tags/aliases/entities); structural compilation runs there, retina enrichment lands at dream time. Full rebuild from canon on open when the compiler-version stamp is stale; a `doctor` check reports staleness. Because a schema bump makes the DB unreadable to older binaries, any live redeploy that migrates keeps a pre-migration DB copy for rollback. A memory with no compiled triggers is still fully reachable via fusion retrieval and deliberate recall — the index is an accelerator, not a door.
