# Memorum — Runtime-Loop Foundation & Local-First Closure (v0.1)
...
3. **Quarantine-resolution CLI.** `memoryd quarantine list` and `memoryd quarantine resolve <id> [--accept-ours | --accept-theirs | --edited]` lift `status: quarantined` (the `blocking_conflicts` source, `reconcile.rs:497-522`), reindex, re-run the rescan in (2), and clear the notification.
