...
|  17 | Lease contention resolution        | simulator    | domain     | Stream H spec §3.2 test #17 (deferred/self-skipped) | Two daemon instances concurrently claiming the same work without correct contention handling.                  |
|  18 | Encrypted tier key rotation        | simulator    | domain     | Stream H spec §3.2 test #18 (deferred/self-skipped) | Rotated encrypted-tier keys breaking fallback reads or leaving decommissioned keys active.                     |
...
- `domain`: Memorum-specific behavioral tests. Tests #13 and #15 are real-harness tests; #14, #17, and #18 are serial because they mutate shared temp git/key state; #16 is parallel-safe. Tests #17 and #18 are authored but deferred in v1 and report `skip_kind: feature_deferred` until their upstream Stream F/D contracts ship.
