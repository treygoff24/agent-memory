...
|  17 | Lease contention resolution        | simulator    | domain     | Stream H spec §3.2 test #17; alpha release-set required | Two daemon instances concurrently claiming the same work without correct contention handling.                  |
|  18 | Encrypted tier key rotation        | simulator    | domain     | Stream H spec §3.2 test #18; alpha release-set required | Rotated encrypted-tier keys breaking fallback reads or leaving decommissioned keys active.                     |
...
- `domain`: Memorum-specific behavioral tests. Tests #13 and #15 are real-harness tests; #14, #17, and #18 are serial because they mutate shared temp git/key state; #16 is parallel-safe. Tests #17 and #18 are required by the alpha release-set and must not report `skip_kind: feature_deferred` in `memorum-eval --harness mock --required-release-set alpha --output json`.
