...
The daemon's *incremental* write path never hits this — the supersede target is already indexed by the time the replacement is written — which is why no existing test caught it.
