...
- **Model: `Qwen/Qwen3-Embedding-0.6B`** (Apache 2.0, ungated, 1024-dim w/ MRL, 32K ctx). Chosen by golden-corpus bench over EmbeddingGemma-300m, Qwen3-4B, and bge-m3: it wins the two metrics encoding Memorum's real failure modes — **trap-rate@5 0.261** (12/46 vs 16–17/46 for all others; ~30% fewer superseded/wrong-project lookalikes surfaced) and **abstention gap 0.332** (vs EmbeddingGemma's 0.137 — Gemma ranks better on nDCG but is *confidently wrong* on no-answer queries, the worst property for unprompted context injection). Bench harness preserved at `fixtures/golden/_embed_bench/`.
- **Lane: fastembed candle path** — `fastembed = "=5.16.0"` with `features = ["qwen3", "metal"]`, `Qwen3TextEmbedding` struct (not the ONNX `TextEmbedding` API). ⚠️ Metal device (`Device::new_metal(0)`) is wired but undocumented in fastembed examples — smoke-test first; `accelerate` (Apple BLAS, CPU) is the fallback feature if Metal misbehaves. Sync API → drain loop under `spawn_blocking`. Bench latency on this M4 Max: ~103ms/query fp16-MPS via Python (production candle lane expected similar or better; informational only).
- **Asymmetric prompts are part of the contract:** Qwen3 query embeddings take the model-card instruction prompt; document/chunk embeddings are plain. The provider trait must expose `embed_query` vs `embed_document` — collapsing them measurably degrades retrieval.
- **Default triple: `("fastembed-candle", "Qwen/Qwen3-Embedding-0.6B", 1024)`** — provider, model_ref, AND dimension all change vs the spec's literal `embeddinggemma` string (768→1024). Approved by Trey 2026-06-09 ("adapt ur spec and plan accordingly"). Contract-touching per invariant 3 → ship the dated stream-a spec amendment together with Task 3.0's code, not before.
- **Acquisition:** download-on-first-use via hf-hub cache → `<runtime_root>/models`, progress shown during `memoryd init`. **Never bundle weights.** Apache 2.0 noted in init output + runbook (simpler story than the prior Gemma license boundary).
...
#### Design as shipped — amendment (2026-06-10)

**Placement: `crates/memorum-coordination/src/harness.rs`.** Verified no layering violation: `memorum-coordination`'s `Cargo.toml` depends only on `memory-substrate`, `serde`, `tokio`, etc. — it has **no** dependency on `memoryd`, and `memoryd` already depends on `memorum-coordination`. The descriptor registry needs nothing from `memoryd`: `CliSpec` carries a program-name string and `ImporterId` is an opaque token that only *names* a parser. The concrete `HarnessCli` trait (dream) and the import `Harness` enum stay as code in `memoryd` and key off the descriptor's `id`. So no new crate is needed and no upward edge into `memoryd` is introduced.

**Types (in `harness.rs`, re-exported from the crate root):**

- `Coordination { Full, ObserveOnly }` — `Default = ObserveOnly` (privilege-escalation guard: unknown harnesses are never Full).
- `McpConfig { JsonAtPath, TomlAtPath }` — the *shape* of the MCP config doc; the wiring site resolves the concrete path. `mcp_config: Option<McpConfig>` (`None` = no MCP surface).
- `CliSpec { program: String }` — executable name resolved on `PATH`.
- `ImporterId(String)` — opaque importer name; config may reference but cannot define one.
- `HarnessDescriptor { id, aliases: Vec<String>, tier: u8, coordination, cli: Option<CliSpec>, mcp_config: Option<McpConfig>, importer: Option<ImporterId> }`. `matches(candidate)` is case-folded + trimmed against `id` and every alias.
- `HarnessRegistry { descriptors: Vec<HarnessDescriptor> }` with `builtin()`, `with_config_overrides(full_coordination_harnesses: Option<&[String]>, additional_descriptors: &[HarnessDescriptor])`, `resolve(id_or_alias) -> Option<&HarnessDescriptor>`, `coordination(id) -> Coordination`, `is_full_coordination(id) -> bool`.

**Built-ins (the alias reconciliation that unifies the four spellings):**

- `claude-code`: aliases `["claude"]`, tier 1, `Full`, cli `claude`, `JsonAtPath`, importer `claude-code`. This single descriptor reconciles `"claude-code"` (coordination/import) with `"claude"` (dream-registry key, MCP `HarnessTarget::Claude`).
- `codex`: aliases `["codex-cli"]`, tier 1, `Full`, cli `codex`, `TomlAtPath`, importer `codex`. Reconciles `"codex"` with the `"codex-cli"` spelling in the retired allowlist.

**Data vs. code split.** Coordination capability is data: `CoordinationConfig.full_coordination_harnesses: Option<Vec<String>>` (loaded by the existing `coordination_config.rs` path, no new loader) *replaces* the full-coordination set when present — every descriptor matching a listed name becomes `Full`, every other known descriptor becomes `ObserveOnly`, and an unlisted-but-named spelling becomes a synthetic Tier-3 `Full` descriptor (so adding a full-coordination Tier-3 harness needs zero code). `config.harness_registry()` produces the per-config registry. Additional full `HarnessDescriptor`s (with their own cli/mcp data) are also loadable via `with_config_overrides`'s second argument. Importers stay code: an `ImporterId` only names a parser that must exist in `memoryd`.

**Migration approach (behavior-preserving).** `session.rs` resolves capability through an optional `SessionContext.harness_registry` (defaults to `builtin()`, injected with the config registry at the startup/delta construction sites). The dream `HarnessCliRegistry` keeps its `"claude"`/`"codex"` adapter keys (load-bearing for the status/doctor surface and `dream_harness_cli` assertions) but `get()` now also resolves any descriptor alias, so `get("claude-code")` reaches the Claude adapter — additive, removes nothing. `HarnessTarget` and import `Harness` keep their load-bearing serde/wire tokens unchanged and gain `from_identifier()` / `descriptor_id()` that route through the shared registry.
