...
**Status:** r5 — foundry execution plan, three review rounds complete, round-3 verdict READY-WITH-EDITS with all six edits applied. **Awaiting Trey's approval + six decision points. Do not execute until approved.**
...
- **r5 (2026-07-10):** round-3 convergence re-read (Sol xhigh vs r4): zero blockers, zero structural findings, verdict READY-WITH-EDITS; all six edits applied — side-independent cue truncation (two-clone convergence), staged-replacement non-servability + crash-point assertions (W3), `write-note` + `quarantine resolve --edited` added to privacy entrypoints (hardcoded-`Trusted` audit), crate-scoped `--locked` check (CPU discipline), API prompt-vs-desk surface wording, corrected wave-graph edges. Sol certified the baseline₁ A/B protocol and journaled merge design as sound.
...
W0 (benchmarks: ingest + baseline₀) ──┐
                                      ├─→ W4-prep (eval-corpus backfill via W2 job) ─→ W4 (fusion; A/B baseline₁ → holdout)
W2 (substrate + generation job) ──────┤
  └─────→ W3 (merge-on-dream; fixture-testable at W2, live-value after W5)
            ⇢ (data-only edge: W1's lineage sets feed W3's exclusion fence)
W1 (import fix) — independent build; parallel with W0
W5 (live-corpus backfill; separately approved; uses W2 job) — after W2, independent of W4
W6 (privacy spike memo) — independent
```

W0 ∥ W1 in parallel (integration: first-done fast-forwards `main`; second rebases before its coordinator commit and runs `cargo check -p <its touched packages> --locked` (crate-scoped — never a bare workspace build, per CPU discipline); lockfile conflicts resolved coordinator-only via targeted `cargo update -p`). W1→W3 is a *data* dependency only (W3's fences exclude W1 lineage). **Live W5 is NOT a prerequisite for any W3/W4 gate.**
...
1. **Stream A frontmatter** — `abstraction` (≤8 words) + `cues` (0–3 phrases): **version-bump decision (DP1)**. Semantics: optional fields; normalization (case, charset, length); merge rules — `abstraction` ours-wins (peer's may be better; dream repair closes the window — accepted risk), `cues` union → canonicalize + case-fold dedup → **side-independent total order → keep first 3** (no ours-first or any Git-side-dependent priority — opposite merge directions must retain identical cues); two-clone convergence tests.
...
3. **Privacy composition — all write entrypoints enumerated:** write, `write-note`, supersede, import execute, `review approve` (currently writes `Trusted` — must compose), `quarantine resolve --edited` (also hardcodes `Trusted` today, `handlers/quarantine.rs:61-80` — a secret added during conflict resolution would activate unclassified), dream fragment→memory, dream abstraction-compile, backfill; plus an audit of every hardcoded-`Trusted` canonical rewrite. Each classifies the **combined** body/title/summary/abstraction/cues payload; strictest outcome controls; `secret` refuses before any disk effect; generated-abstraction-turns-out-sensitive → **drop abstraction, keep body** (fail-closed). **Sensitivity upgrade revokes API-lane vectors** for that memory and re-enqueues held-local. Tests: secret-in-cue, sensitive-abstraction-on-public-body, upgrade-revocation.
...
- **Surface split (r4 pre-decision, r5 wording):** the **API-lane hook paths are unchanged** — API prompt/delta recall keeps today's two-lane FTS + chunk-vector under today's budgets (750ms embed inside the 800ms hook deadline leaves no tail residue for more lanes; measured, not discovered at gate time), and API desk/startup recall keeps today's non-fusion path (it receives no vector-recall context today — `handlers/memory_ops.rs:63-73`). The new lanes serve: (a) the **local-lane hook path**, under a shared-deadline formula `embed_budget = min(lane timeout, deadline − measured_fusion_reserve)`; (b) the **`memoryd search` pull path** on both lanes, under its own longer timeout. Raising the hook deadline for fusion-bearing cues stays an option inside the spec if local-lane measurement demands it.
