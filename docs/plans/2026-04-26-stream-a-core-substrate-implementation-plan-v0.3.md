# Stream A Core Substrate Implementation Plan (v0.3)
...
- **v0.2 (2026-04-26):** spec bumped to v1.1; introduced the worktree/lockfile model, script contracts, agentlinters SHA pin, classification routing tests, embedding triple migration tests, `WatchSubscription` drop test, baseline-driven perf regression detection, and the spec acceptance coverage manifest.
- **v0.3 (2026-04-26):** plan-reviewer remediation pass. Closes six blockers and six sharper risks from the v0.2 review. Fixes: stale v0.1 path/spec references in Task 0 and the duplicate-check command; wrong `STREAM_A_SPEC_VERSION` constant; contradictory `bench-regression-check.sh` `runs == 0` vs `runs < 9` rule (with a first-release bootstrap path that satisfies §17.7 criterion 6); replaces `cargo generate-lockfile` with `cargo build --workspace --locked` + targeted `cargo update`; gives `memory-test-support/src/convergence.rs` an explicit owner; splits Task 6 vector-reconciliation tests so Task 6 owns `vector::reconcile_*` helpers and Task 10 owns the integration; whitelists `Substrate::watch` in the no-blocking-sync test; gates `WatchSubscription` OS-resource test to Linux only; rewrites cancellation test 2 as a tri-state branch assertion; narrows `scripts/check.sh` invocation inside worktrees; broadens the spec coverage manifest parser to also pick up `### N.M Overall acceptance` and the §17.8 `Acceptance:` sub-block; adds `RUSTDOCFLAGS=-D warnings` to the doc gate; specifies the manifest-test spec-file path resolution and the smoke-tier seed origin.
...
- Current repo state before this plan: Stream A spec files are uncommitted user work; do not overwrite them. This plan supersedes the v0.2 plan at `docs/plans/2026-04-26-stream-a-core-substrate-implementation-plan-v0.2.md` and the v0.1 plan at `docs/plans/2026-04-25-stream-a-core-substrate-implementation-plan.md` (both kept for history). The active plan path is `docs/plans/2026-04-26-stream-a-core-substrate-implementation-plan-v0.3.md`.
...
- The orchestrator, during integration, **does not** regenerate `Cargo.lock` from scratch (`cargo generate-lockfile` would re-resolve every dependency to the newest compatible version, ignoring the worker's choices and producing unreviewable diffs). The integration sequence is instead:
  1. Apply the worker's `Cargo.toml` change to the integrated trunk.
  2. `cargo build --workspace --locked --offline=false` — fails fast if the worker's lockfile entries are inconsistent with the integrated state.
  3. If `--locked` fails because a dependency newly added by this worker is missing from the trunk lockfile, run `cargo update -p <crate>` for **each crate the worker actually added or bumped** (recovered from the diff), then re-run `cargo build --workspace --locked`.
  4. Commit the resulting `Cargo.lock` as a separate commit alongside the worker's `Cargo.toml` commit.
- **Conflicting transitives between two parallel tasks.** If Task A and Task B both add a different version of the same transitive (or pull in mutually incompatible versions of a shared dep), `cargo build --workspace --locked` fails during integration of the second task. The orchestrator's response is fixed: (a) revert the second integration, (b) open a remediation subtask owned by the second-integrated task with the conflict diff in the prompt, (c) the worker reconciles by aligning to the version already on `main` or proposing an explicit upgrade in the same task. Workers do not perform their own conflict reconciliation across other tasks' branches.
...
### Workers run narrow gates, not `scripts/check.sh`

Inside a task worktree, the workspace contains module stubs from every task already wired by Task 2 (`pub mod git`, `pub mod index`, etc.) plus only the worker's own implementation. Running `cargo test --workspace` would execute tests against unstarted-task stubs and fail for the wrong reason. So:

- **Workers** run only their per-task narrow gate (the explicit `cargo test -p memory-substrate <filter>` invocation listed in the task's verification plan, plus fmt/clippy/dylint).
- **The orchestrator** runs `scripts/check.sh` only after `integrate-task-worktree.sh` fast-forwards `main`, and only when the integration count has reached a meaningful checkpoint (typically: after the last parallel-lane task in a fan-in to the next sequential task). The orchestrator may run `cargo test --workspace` more frequently — that's fine because integrated trunk has all wired modules in their committed state.
- **`integrate-task-worktree.sh` runs `scripts/check.sh` inside the integrated trunk worktree, not inside the task worktree** (the script is invoked after fast-forward, so the working tree state matches `main` post-integration).
...
- `--tier smoke`: 1K-memory corpus, 5 repetitions per metric, seed `memory-test-support::perf::SEED_SMOKE` (constant `0xA17_50f7`). Target wall-clock: < 60s. Used by `scripts/check.sh`. Does not gate on regression; only fails on outright crash or missing target.
- `--tier release`: 10K-memory corpus per spec §17.6, 9 repetitions per metric, seed `memory-test-support::perf::SEED_RELEASE` (constant `0xA17_5e1ea5e`). Writes the full result JSON including hardware/OS/filesystem/SQLite pragma rows.

Both `SEED_SMOKE` and `SEED_RELEASE` are defined in `memory-test-support/src/perf.rs` and land in Task 11 alongside `synthetic_vectors`. Until Task 11 lands, `scripts/check.sh` is allowed to be incomplete (per the existing Task 1 caveat at the start of "Full local gate"); the smoke-tier bench is not part of any task's narrow gate before Task 11.
...
- Refuses to run if `current.runs < 9` (the results JSON must reflect ≥9 repetitions per spec §17.6).
- Three baseline states, in priority order:
  1. **Baseline file missing** → exit non-zero with `error: bench/baseline.<profile>.json not found; commit a placeholder via Task 1 scaffolding`.
  2. **Baseline present with `runs == 0`** (the Task 1 placeholder; bootstrap path) → emit a `warning: baseline is a placeholder; first-release bootstrap path active`, copy the current results into a fresh baseline JSON written to `--baseline.proposed`, exit `0`. The orchestrator's release process (Task 13) reviews `*.proposed`, accepts it as the inaugural baseline by `git mv` + commit (the explicit human-authored commit per spec §17.6 / §18.9), and re-runs the gate. The second run then has a real baseline and follows the standard regression check.
  3. **Baseline present with `runs >= 9`** → run the regression check. This is the steady-state path.
- This satisfies §17.7 criterion 6 on first release: the inaugural release tag's gate run produces and commits the baseline; subsequent tags compare against it.
- Output includes a per-metric pass/fail table and the exact p95 deltas for the release-review doc.
...
`spawn-task-worktree.sh` runs `git worktree add -b stream-a/task-<id>-<slug> ../agent-memory-wt/task-<id> main`. `integrate-task-worktree.sh <task-id> [--gate narrow|checkpoint|full]` rebases the task branch onto `main`, runs the task's declared narrow gate by default, fast-forwards `main`, and removes the worktree on success. It never runs `cargo generate-lockfile` and never regenerates `Cargo.lock` from scratch. If a task changes a `Cargo.toml`, the orchestrator runs `cargo build --workspace --locked --offline=false`; if the lockfile is missing newly added dependencies, the orchestrator performs targeted `cargo update -p <crate>` for only crates added or bumped by that task, then reruns the locked build. `--gate checkpoint` may run `scripts/check.sh` only after a fan-in where all referenced downstream scripts exist. `--gate full` is allowed only from Task 11 onward. Both helpers refuse to run if `main` has uncommitted changes.
...
**Parallel:** no
**Blocked by:** none
**Owned files:** `docs/plans/2026-04-26-stream-a-core-substrate-implementation-plan-v0.3.md`
...
- Verify exists: `docs/plans/2026-04-26-stream-a-core-substrate-implementation-plan-v0.3.md`
- Read: `docs/specs/stream-a-core-substrate-v1.1.md`
...
```bash
rg -n "TBD|TODO|open|deliberately does not decide|Acceptance signals|Overall acceptance" docs/specs/stream-a-core-substrate-v1.1.md
```
...
```bash
test -f docs/plans/2026-04-26-stream-a-core-substrate-implementation-plan-v0.3.md
rg -n "\*\*Owned files:\*\*" docs/plans/2026-04-26-stream-a-core-substrate-implementation-plan-v0.3.md
```
...
**Initial baseline files:** `bench/baseline.linux-x86_64.json` and `bench/baseline.darwin-arm64.json` are created in Task 1 with placeholder bodies (`{ "schema": 1, "metrics": {}, "noise_floor_ms": 2.0, "runs": 0 }`). The first real values are produced by Task 11's bench harness and promoted into the baseline by Task 13's first-release bootstrap path described in the `bench-regression-check.sh` contract above (the inaugural release tag's gate run accepts the proposed baseline via an explicit human-authored commit and re-runs the gate against it, satisfying spec §17.7 criterion 6).
...
Create the three crates and expose `pub const STREAM_A_SPEC_VERSION: &str = "1.1";`.
...
RUSTDOCFLAGS="-D warnings" cargo doc -p memory-substrate --no-deps
```

The `RUSTDOCFLAGS=-D warnings` flag is required by spec §17.7 criterion 5 ("docs build without warnings"). It must appear in the release gate, in `stream-a-ci.yml`, and in the per-task verification command anywhere `cargo doc` is invoked.
...
### Shared public seam change protocol

After Task 2, downstream tasks may not edit `api.rs`, `error.rs`, or `model.rs` opportunistically. If a downstream task needs a public type, enum variant, or method signature change, the orchestrator opens a serialized seam patch with a compile test and integrates that patch before the worker resumes. Submodule-owned internal types should stay inside the owning module; `model.rs` and `error.rs` remain the stable public contract surface.
...
**Parallel:** yes
**Blocked by:** Task 2 for layout/config/sequence slices; Task 3 for cross-file validation and duplicate-ID repair slices that parse frontmatter. Execute Task 4 as 4a (layout/config/sequence) then 4b after Task 3 lands.
...
**Out of scope:** SQLite indexing internals; Task 6 plugs into the transaction hooks. Task 5 owns durable file/event semantics through an injected index-transaction seam or test double; real SQLite no-effect assertions for classification refusals are integration-owned by Task 10/11 after Task 6 exists.
...
**Parallel:** yes after Tasks 3-4
**Blocked by:** Tasks 3, 4
**Owned files:** `crates/memory-substrate/src/index/**`, `crates/memory-substrate/tests/index_*.rs`, `crates/memory-substrate/benches/**`, `fixtures/index/**`
...
- Generated bench result files are not owned by Task 6 and remain Task 11/release evidence outputs.
...
Tests in this task (unit-level on the `vector::reconcile_*` helpers):

- `update_embedding_rejects_stale_chunk_hash`
- `delete_tombstone_or_sensitivity_change_purges_vectors_after_reconciliation`
- `vector_reconcile_orphans_deletes_vectors_with_no_chunk_meta` — exercises `vector::reconcile_orphans()` directly.
- `vector_reconcile_missing_requeues_jobs_for_chunks_without_vectors` — exercises `vector::reconcile_missing()` directly.

**Ownership split with Task 10.** Spec §10.2.1 startup reconciliation has two layers: (a) the per-pass vector helpers that walk `chunk_embedding_meta` ↔ `VectorStore::list_chunk_ids()`, and (b) the orchestration that runs them inside `Substrate::open` per spec §13.5.1. Task 6 owns (a) — pure functions inside `crates/memory-substrate/src/index/vector.rs` with the names `reconcile_orphans`, `reconcile_missing`, and `reconcile_pending_jobs`. Task 10 owns (b) — the call site inside `runtime/reconcile.rs` and the integration tests `startup_reconciliation_deletes_orphan_vectors` and `startup_reconciliation_requeues_missing_vectors`. Task 6's tests construct an `Index` plus a stub `VectorStore` directly and call the helpers; they do not invoke `Substrate::open`.
...
- `watch_subscription_unsubscribe_releases_os_resources` — Linux only (`#[cfg(target_os = "linux")]`). Counts entries in `/proc/self/fd` before subscription and after `unsubscribe()` / `drop`; asserts delta returns to zero. macOS is not covered by this test because FSEventStream isn't a unix file descriptor and `lsof | grep FSEvents` is unreliable across macOS releases — the spec §11.4 acceptance signal is satisfied on macOS by `dropping_watch_subscription_handle_releases_resources_synchronously` plus a Drop-side-effect test using a `Watcher` test double.
- `dropping_watch_subscription_handle_releases_resources_synchronously` — cross-platform, uses a `notify::Watcher` test double that increments a counter on `Drop`. Asserts the counter increments exactly once when the handle is dropped without calling `unsubscribe()`.
...
**Blocked by:** Tasks 4, 5, 6, 9b
**Owned files:** `crates/memory-substrate/src/git/**`, `crates/memory-substrate/tests/git_*.rs`, `fixtures/git/**`, `scripts/two-clone-convergence.sh`, `crates/memory-test-support/src/convergence.rs`, `crates/memory-test-support/tests/convergence.rs` (added under the `memory-test-support` steward rule — Task 8's spawn prompt declares this file lock to the orchestrator before Task 11 spawns)
...
- vector reconciliation runs before accepting writes — `startup_reconciliation_deletes_orphan_vectors` and `startup_reconciliation_requeues_missing_vectors` are owned here and exercise `Substrate::open` end-to-end against fixtures seeded with orphan/missing vectors. They call into Task 6's `vector::reconcile_*` helpers via the public `Substrate::open` path, not directly.
...
- `cancel_between_rename_and_parent_fsync_reconciles_via_one_of_three_branches` — drop the future between §8.3 steps 10 and 11; subsequent `Substrate::open` must take exactly one of three branches per spec §13.5.1 step 2, and the test must assert which branch fired and verify the corresponding canonical-state predicate:
  - **Branch A — cleaned away:** the temp-renamed file was already removed by the cancel-cleanup hook before reconciliation. Post-open canonical state matches the no-write baseline; no events emitted beyond `StartupReconciliationCompleted { reindexed: 0, ... }`.
  - **Branch B — ingested as human edit:** the file is valid memory Markdown. Post-open canonical state contains a new committed memory with the operation_id-derived path; one `WriteCommitted` (or equivalent ingestion event) and `StartupReconciliationCompleted { reindexed: 1, ... }` are emitted. The new memory's `author.kind` reflects the offline-edit ingestion branch (not the original requester).
  - **Branch C — operator-quarantined:** the file is invalid or has conflict markers. Post-open canonical state contains the file under `~/.memoryd/quarantine/<startup-ts>/` and `OperatorRepairRequired` is emitted; substrate refuses writes until the operator clears it.

  Each branch has its own fixture; the parent test name is a roll-up `cancel_between_rename_and_parent_fsync_reconciles_via_one_of_three_branches` that asserts exactly one of `{branch_a_cleanup, branch_b_ingested, branch_c_quarantined}` fired per fixture. The fault-injection harness chooses which branch to exercise by seeding the temp file's content (valid Markdown vs invalid vs already-deleted) before triggering the cancel.
...
- `no_public_sync_method_performs_blocking_git_sqlite_or_network_work` — clippy-adjacent test that walks the public API and refuses any non-`async`, non-`_blocking`-suffixed method that touches `git::`, `index::`, `events::`, or `notify::` modules. Allowlisted exception: `Substrate::watch(&self) -> Result<WatchSubscription, WatchError>` is intentionally synchronous per spec §16.5 (subscription setup is a one-shot, non-blocking call into `notify::Watcher::new`). The test maintains a `SYNC_API_ALLOWLIST: &[&str] = &["watch"]` and a comment pointing at the spec section that justifies each entry. New entries require a spec citation in the allowlist comment.
...
- **Spec file location.** Resolve the spec via `const SPEC_PATH: &str = concat!(env!("CARGO_MANIFEST_DIR"), "/../../docs/specs/stream-a-core-substrate-v1.1.md");`. The test reads it at runtime (not `include_str!`) so a spec edit doesn't require recompilation.
- **Parser scope.** Walk the spec line-by-line and collect bullets from every section that matches one of:
  - `### Acceptance signals`
  - `### N.M Acceptance signals` (e.g. `### 6.13 Acceptance signals`)
  - `### N.M Overall acceptance` (catches §17.7 — the release-gate criteria)
  - The `Acceptance:` sub-block inside §17.8 itself (matched by a `^Acceptance:$` line followed by a bullet list, terminated by the next `^---$` or `^## ` boundary)
- Maintain a structured `SPEC_COVERAGE: &[SpecCoverageEntry]` where each entry includes `section`, `bullet_hash` (sha256 of bullet text after stripping the leading `- ` and trailing whitespace, truncated to 16 hex chars), and `test_path::test_name`.
- Fail the test if any spec bullet has no manifest entry, or any manifest entry references a missing test, or any manifest entry references a hash that no longer appears in the spec (catches deleted bullets).
...
- `stream-a-ci.yml` `[CI]` — runs on every push and PR: fmt, clippy, dylint, unit/integration tests, `RUSTDOCFLAGS=-D warnings cargo doc --workspace --no-deps` (spec §17.7 criterion 5), Oxfmt/Oxlint, Specgate, `rust-boundary-check.sh`, `spec_coverage_manifest`, `event_kind_coverage`, `error_variant_coverage`, durability probe matrix, two-clone convergence (smoke tier).
...
rg -n "§17.7|cargo test --workspace --release|merge_driver|two-clone|bench/results\.linux-x86_64\.json|bench/results\.darwin-arm64\.json|Independent review" docs/reviews/stream-a-final-review.md
```
...
```bash
rg '\*\*Owned files:\*\*' docs/plans/2026-04-26-stream-a-core-substrate-implementation-plan-v0.3.md \
  | sed 's/.*\*\*Owned files:\*\* *//' \
  | tr ',' '\n' \
  | sed 's/`//g' \
  | sed 's/^[[:space:]]*//;s/[[:space:]]*$//' \
...
spawn plan_checker for docs/plans/2026-04-26-stream-a-core-substrate-implementation-plan-v0.3.md
```
