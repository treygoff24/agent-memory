# Agent harness memory, context, and state engineering handbook, v2.1
...
v2.1 is an incremental tightening of v2 against one more Deep Research pass and fresh primary-source verification. The core architecture from v1 and v2 survives. Structured Markdown memory, frontmatter discipline, entity indexing, passive recall, smart compaction, tool output guards, subagent memory, and dreaming are all still the foundation. What v2 added was governance, evaluation, lifecycle, expanded retrieval, cache-stable assembly, prospective memory, and cost modeling. What v2.1 changes is narrower. A handful of facts got corrected against live documentation (the bootstrap caps were the biggest). A set of named attack families, benchmarks, and framework-landscape specifications got added. And a few chapters got reframed to acknowledge that the mature production implementations the handbook describes don't yet fully exist; the schema below is a *target architecture* informed by what shipping systems have partially implemented, not a documentation of current industry standard.

v2.1 also reflects something I learned running this research loop. Multiple AI-mediated research passes, including my own earlier verification against Context7, each surfaced real signal and each introduced at least one confident error. The live vendor documentation was the only consistently reliable source. That lesson deserves its own short section before the rest of the document, because the same discipline the governance chapter recommends for an agent's memory applies to the construction of this handbook and to anyone trying to adapt it.
...
7. Evaluation.
8. Structured memory substrate.
9. Entity index and passive recall.
10. Compaction as state reconstruction.
11. Tool output guards and artifact stubs.
12. Subagent continuity.
13. Retrieval architecture and the pattern menu.
14. Context assembly and cache stability.
15. Prospective memory.
16. Cost modeling.
17. Sleep-time synthesis and dreaming.
18. Failure modes we learned the hard way.
19. Current framework landscape.
20. Implementation phasing, revised.
21. Source map.
...
A closed tag vocabulary matters. Let tags sprawl and you end up with synonyms fragmenting the index. The human equivalent is remembering the same concept under five slightly different names.
...
Phase 8. Subagent persistence. If the harness delegates, add agent-specific memory and writeback with the same promotion gates as any other source. Path-scoped ACLs modeled on LangChain Deep Agents.

Phase 9. Cache-stable context assembly. Place stable content first, dynamic content last. Update recall only when triggering entities change. Budget the cache boundary explicitly.

Phase 10. Prospective memory. Add the external scheduler and the prospective memory surface. Remember this is not a mature category in shipping systems; you're innovating.

Phase 11. Sleep-time synthesis. Only after the substrate is clean enough to be worth synthesizing. Dreaming on messy memory amplifies noise.

Phase 12. Ongoing evaluation and drift audits. Periodic ground-truth refreshes. Longitudinal regression tracking. The agent asking the user "is this still right?" on high-stakes stored beliefs.
